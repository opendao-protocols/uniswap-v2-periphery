...
import ExampleSlidingWindowOracle from '../build/IncentivisedSlidingWindowOracle.json'
...
describe.only('IncentivisedSlidingWindowOracle', () => {
  ...
  describe('#update', () => {
    ...
    it('reverts when second update in the same period', async () => {
      await slidingWindowOracle.update(token0.address, token1.address, overrides)
      await expect(slidingWindowOracle.update(token0.address, token1.address, overrides)).to.be.revertedWith(
        'SlidingWindowOracle: ALREADY_UPDATED_THIS_PERIOD'
      )
    })
    ...
    it('reverts when second update at end of timeslot', async () => {
      await slidingWindowOracle.update(token0.address, token1.address, overrides)
      // first hour still
      await mineBlock(provider, startTime + 3599)
